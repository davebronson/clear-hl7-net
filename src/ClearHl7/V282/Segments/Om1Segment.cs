...
namespace ClearHl7.V282.Segments
{
    ...
    public class Om1Segment : ISegment
    {
        ...
        public void FromDelimitedString(string delimitedString, Separators separators)
        {
            ...
            ReplacementProducersServiceTestObservationId = segments.Length > 52 && segments[52].Length > 0 ? segments[52].Split(seps.FieldRepeatSeparator, StringSplitOptions.None).Select(x => TypeSerializer.Deserialize<CodedWithExceptions>(x, false, seps)) : null;
            PriorResultsInstructions = segments.Length > 53 && segments[53].Length > 0 ? segments[53].Split(seps.FieldRepeatSeparator, StringSplitOptions.None).Select(x => TypeSerializer.Deserialize<Text>(x, false, seps)) : null;
            SpecialInstructions = segments.Length > 54 && segments[54].Length > 0 ? TypeSerializer.Deserialize<Text>(segments[54], false, seps) : null;
            TestCategory = segments.Length > 55 && segments[55].Length > 0 ? segments[55].Split(seps.FieldRepeatSeparator, StringSplitOptions.None).Select(x => TypeSerializer.Deserialize<CodedWithExceptions>(x, false, seps)) : null;
            ObservationIdAssociatedWithProducersServiceTestObservationId = segments.Length > 56 && segments[56].Length > 0 ? TypeSerializer.Deserialize<CodedWithExceptions>(segments[56], false, seps) : null;
            TypicalTurnAroundTimeWithUnits = segments.Length > 57 && segments[57].Length > 0 ? TypeSerializer.Deserialize<CompositeQuantityWithUnits>(segments[57], false, seps) : null;
            GenderRestriction = segments.Length > 58 && segments[58].Length > 0 ? segments[58].Split(seps.FieldRepeatSeparator, StringSplitOptions.None).Select(x => TypeSerializer.Deserialize<CodedWithExceptions>(x, false, seps)) : null;
            AgeRestriction = segments.Length > 59 && segments[59].Length > 0 ? segments[59].Split(seps.FieldRepeatSeparator, StringSplitOptions.None).Select(x => TypeSerializer.Deserialize<NumericRange>(x, false, seps)) : null;
        }
