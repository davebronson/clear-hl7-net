...
namespace ClearHl7.V260.Segments
{
    ...
    public class QrdSegment : ISegment
    {
        ...
        public void FromDelimitedString(string delimitedString)
        {
            ...
            WhatSubjectFilter = segments.Length > 9 && segments[9].Length > 0 ? segments[9].Split(separator).Select(x => TypeHelper.Deserialize<CodedWithExceptions>(x, false)) : null; ;
            WhatDepartmentDataCode = segments.Length > 10 && segments[10].Length > 0 ? segments[10].Split(separator).Select(x => TypeHelper.Deserialize<CodedWithExceptions>(x, false)) : null; ;
