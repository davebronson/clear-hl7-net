...
namespace ClearHl7.V290.Segments
{
    ...
    public class PeoSegment : ISegment
    {
        ...
        public PeoSegment FromDelimitedString(string delimitedString)
        {
            ...
            EventIdentifiersUsed = segments.Length > 1 ? segments.ElementAtOrDefault(1).Split(separator).Select(x => new CodedWithExceptions().FromDelimitedString(x)) : null;
            EventSymptomDiagnosisCode = segments.Length > 2 ? segments.ElementAtOrDefault(2).Split(separator).Select(x => new CodedWithExceptions().FromDelimitedString(x)) : null;
            ...
            EventLocationOccurredAddress = segments.Length > 7 ? segments.ElementAtOrDefault(7).Split(separator).Select(x => new ExtendedAddress().FromDelimitedString(x)) : null;
            ...
            CauseOfDeath = segments.Length > 18 ? segments.ElementAtOrDefault(18).Split(separator).Select(x => new CodedWithExceptions().FromDelimitedString(x)) : null;
            PrimaryObserverName = segments.Length > 19 ? segments.ElementAtOrDefault(19).Split(separator).Select(x => new ExtendedPersonName().FromDelimitedString(x)) : null;
            PrimaryObserverAddress = segments.Length > 20 ? segments.ElementAtOrDefault(20).Split(separator).Select(x => new ExtendedAddress().FromDelimitedString(x)) : null;
            PrimaryObserverTelephone = segments.Length > 21 ? segments.ElementAtOrDefault(21).Split(separator).Select(x => new ExtendedTelecommunicationNumber().FromDelimitedString(x)) : null;
