...
namespace ClearHl7.V290.Segments
{
    ...
    public class PdaSegment : ISegment
    {
        ...
        public void FromDelimitedString(string delimitedString, Separators separators)
        {
            ...
            DeathCauseCode = segments.Length > 1 && segments[1].Length > 0 ? segments[1].Split(seps.FieldRepeatSeparator, StringSplitOptions.None).Select(x => TypeHelper.Deserialize<CodedWithExceptions>(x, false, seps)) : null;
            DeathLocation = segments.Length > 2 && segments[2].Length > 0 ? TypeHelper.Deserialize<PersonLocation>(segments[2], false, seps) : null;
            ...
            DeathCertifiedBy = segments.Length > 5 && segments[5].Length > 0 ? TypeHelper.Deserialize<ExtendedCompositeIdNumberAndNameForPersons>(segments[5], false, seps) : null;
            AutopsyIndicator = segments.Length > 6 && segments[6].Length > 0 ? segments[6] : null;
            AutopsyStartAndEndDateTime = segments.Length > 7 && segments[7].Length > 0 ? TypeHelper.Deserialize<DateTimeRange>(segments[7], false, seps) : null;
            AutopsyPerformedBy = segments.Length > 8 && segments[8].Length > 0 ? TypeHelper.Deserialize<ExtendedCompositeIdNumberAndNameForPersons>(segments[8], false, seps) : null;
