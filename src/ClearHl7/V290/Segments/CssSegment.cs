...
namespace ClearHl7.V290.Segments
{
    ...
    public class CssSegment : ISegment
    {
        ...
        public CssSegment FromDelimitedString(string delimitedString)
        {
            ...
            StudyScheduledTimePoint = segments.Length > 1 ? new CodedWithExceptions().FromDelimitedString(segments.ElementAtOrDefault(1)) : null;
            StudyScheduledPatientTimePoint = segments.ElementAtOrDefault(2)?.ToNullableDateTime(Consts.DateTimeFormatPrecisionSecond);
            StudyQualityControlCodes = segments.Length > 3 ? segments.ElementAtOrDefault(3).Split(separator).Select(x => new CodedWithExceptions().FromDelimitedString(x)) : null;
