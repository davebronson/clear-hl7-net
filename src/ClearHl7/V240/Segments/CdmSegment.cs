...
namespace ClearHl7.V240.Segments
{
    ...
    public class CdmSegment : ISegment
    {
        ...
        /// <para>Suggested: 0268 Override -&gt; ClearHl7.Codes.V240.CodeOverride</para>
        /// </summary>
        ...
        public void FromDelimitedString(string delimitedString, Separators separators)
        {
            ...
            PrimaryKeyValueCdm = segments.Length > 1 && segments[1].Length > 0 ? TypeHelper.Deserialize<CodedElement>(segments[1], false, seps) : null;
            ChargeCodeAlias = segments.Length > 2 && segments[2].Length > 0 ? segments[2].Split(seps.FieldRepeatSeparator, StringSplitOptions.None).Select(x => TypeHelper.Deserialize<CodedElement>(x, false, seps)) : null;
            ...
            ExplodingCharges = segments.Length > 6 && segments[6].Length > 0 ? segments[6].Split(seps.FieldRepeatSeparator, StringSplitOptions.None).Select(x => TypeHelper.Deserialize<CodedElement>(x, false, seps)) : null;
            ProcedureCode = segments.Length > 7 && segments[7].Length > 0 ? segments[7].Split(seps.FieldRepeatSeparator, StringSplitOptions.None).Select(x => TypeHelper.Deserialize<CodedElement>(x, false, seps)) : null;
            ActiveInactiveFlag = segments.Length > 8 && segments[8].Length > 0 ? segments[8] : null;
            InventoryNumber = segments.Length > 9 && segments[9].Length > 0 ? segments[9].Split(seps.FieldRepeatSeparator, StringSplitOptions.None).Select(x => TypeHelper.Deserialize<CodedElement>(x, false, seps)) : null;
            ResourceLoad = segments.Length > 10 && segments[10].Length > 0 ? segments[10].ToNullableDecimal() : null;
            ContractNumber = segments.Length > 11 && segments[11].Length > 0 ? segments[11].Split(seps.FieldRepeatSeparator, StringSplitOptions.None).Select(x => TypeHelper.Deserialize<CompositeIdWithCheckDigit>(x, false, seps)) : null;
