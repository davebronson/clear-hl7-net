...
namespace ClearHl7.V240.Segments
{
    ...
    public class Pd1Segment : ISegment
    {
        ...
        /// <para>Suggested: 0441 Immunization Registry Status -&gt; ClearHl7.Codes.V240.CodeImmunizationRegistryStatus</para>
        /// </summary>
        ...
        public void FromDelimitedString(string delimitedString)
        {
            ...
            LivingDependency = segments.Length > 1 && segments[1].Length > 0 ? segments[1].Split(separator) : null;
            LivingArrangement = segments.Length > 2 && segments[2].Length > 0 ? segments[2] : null;
            PatientPrimaryFacility = segments.Length > 3 && segments[3].Length > 0 ? segments[3].Split(separator).Select(x => TypeHelper.Deserialize<ExtendedCompositeNameAndIdNumberForOrganizations>(x, false)) : null;
            PatientPrimaryCareProviderNameIdNo = segments.Length > 4 && segments[4].Length > 0 ? segments[4].Split(separator).Select(x => TypeHelper.Deserialize<ExtendedCompositeIdNumberAndNameForPersons>(x, false)) : null;
            ...
            DuplicatePatient = segments.Length > 10 && segments[10].Length > 0 ? segments[10].Split(separator).Select(x => TypeHelper.Deserialize<ExtendedCompositeIdWithCheckDigit>(x, false)) : null;
            ...
            PlaceOfWorship = segments.Length > 14 && segments[14].Length > 0 ? segments[14].Split(separator).Select(x => TypeHelper.Deserialize<ExtendedCompositeNameAndIdNumberForOrganizations>(x, false)) : null;
            AdvanceDirectiveCode = segments.Length > 15 && segments[15].Length > 0 ? segments[15].Split(separator).Select(x => TypeHelper.Deserialize<CodedElement>(x, false)) : null;
