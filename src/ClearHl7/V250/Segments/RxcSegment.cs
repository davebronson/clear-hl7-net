...
namespace ClearHl7.V250.Segments
{
    ...
    public class RxcSegment : ISegment
    {
        ...
        /// <para>Suggested: 0166 RX Component Type -&gt; ClearHl7.Codes.V250.CodeRXComponentType</para>
        /// </summary>
        ...
        public void FromDelimitedString(string delimitedString, Separators separators)
        {
            ...
            ComponentCode = segments.Length > 2 && segments[2].Length > 0 ? TypeSerializer.Deserialize<CodedElement>(segments[2], false, seps) : null;
            ComponentAmount = segments.Length > 3 && segments[3].Length > 0 ? segments[3].ToNullableDecimal() : null;
            ComponentUnits = segments.Length > 4 && segments[4].Length > 0 ? TypeSerializer.Deserialize<CodedElement>(segments[4], false, seps) : null;
            ComponentStrength = segments.Length > 5 && segments[5].Length > 0 ? segments[5].ToNullableDecimal() : null;
            ComponentStrengthUnits = segments.Length > 6 && segments[6].Length > 0 ? TypeSerializer.Deserialize<CodedElement>(segments[6], false, seps) : null;
            SupplementaryCode = segments.Length > 7 && segments[7].Length > 0 ? segments[7].Split(seps.FieldRepeatSeparator, StringSplitOptions.None).Select(x => TypeSerializer.Deserialize<CodedElement>(x, false, seps)) : null;
